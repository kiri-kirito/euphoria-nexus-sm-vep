...
export default function BundleWishlistButton({ productIds, className = "" }: Props) {
  ...
  useEffect(() => {
    if (!user?.id) return;
    Promise.all([
      isInWishlist(user.id, productIds[0]),
      isInWishlist(user.id, productIds[1])
    ]).then(([s1, s2]) => setSaved(s1 && s2));
  }, [user?.id, productIds]);
  ...
  const toggle = async () => {
    ...
    if (saved) {
      await Promise.all([
        removeFromWishlist(user.id, productIds[0]),
        removeFromWishlist(user.id, productIds[1])
      ]);
      setSaved(false);
    } else {
      await Promise.all([
        addToWishlist(user.id, productIds[0]),
        addToWishlist(user.id, productIds[1])
      ]);
      setSaved(true);
    }
    setLoading(false);
  ...
  return (
    <button
      ...
      onClick={toggle}
      disabled={loading}
      aria-label={saved ? "Remove bundle from wishlist" : "Add bundle to wishlist"}
