...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Attempt file upload if a file was selected via the new file input
    ...
    if (fileInput && fileInput.files && fileInput.files.length > 0) {
      ...
      try {
        ...
        // Ignore error (bucket likely exists)
      }
      ...
      if (!uploadError && uploadData) {
        ...
        console.warn("Storage upload failed, falling back to URL", uploadError);
      }
    }
                ...
                <input 
                  ...
                  onChange={(e) => {
                    if (e.target.files && e.target.files.length > 0) {
                      setFormData({...formData, selectedImage: ''}); // Clear suggestion selection
                    }
                  }}
                />
              </label>
