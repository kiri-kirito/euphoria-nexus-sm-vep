                ...
                <div className="flex-1 border-l border-slate-100 pl-6 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="mt-4 space-y-3">
                      {/* Custom Counter-Offer Input with clear discount and markup options */}
                      <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-2.5">
                        <div className="flex justify-between items-center">
                          <label className="block text-[11px] font-bold text-slate-700">
                            Counter-Offer Price (৳ / unit)
                          </label>
                          <span className="text-[11px] font-extrabold text-primary">
                            Total: ৳{((currentCounter || item.offeredPrice) * item.qty).toLocaleString()}
                          </span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 items-center">
                          ...
                          <button
                            type="button"
                            onClick={() => setCounterInputs(prev => ({ ...prev, [item.id]: Math.round(item.originalPrice * 0.95) }))}
                            className="px-2 py-1 text-[10px] bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-lg border border-emerald-200 transition"
                            title="5% discount from original regular price"
                          >
                            5% off (৳{Math.round(item.originalPrice * 0.95).toLocaleString()})
                          </button>
                          <button
                            type="button"
                            onClick={() => setCounterInputs(prev => ({ ...prev, [item.id]: Math.round(item.originalPrice * 0.90) }))}
                            className="px-2 py-1 text-[10px] bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-lg border border-emerald-200 transition"
                            title="10% discount from original regular price"
                          >
                            10% off (৳{Math.round(item.originalPrice * 0.90).toLocaleString()})
                          </button>
                          <button
                            type="button"
                            onClick={() => setCounterInputs(prev => ({ ...prev, [item.id]: Math.round(item.offeredPrice * 1.05) }))}
                            className="px-2 py-1 text-[10px] bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-lg border border-blue-200 transition"
                            title="+5% higher than buyer's offer"
                          >
                            +5% on Offer (৳{Math.round(item.offeredPrice * 1.05).toLocaleString()})
                          </button>
                        </div>
                        ...
                        <div className="flex gap-2 items-center pt-1">
                          <div className="relative flex-1">
                            ...
                            <input
                              ...
                              className="w-full bg-white border border-slate-300 rounded-xl pl-7 pr-3 py-2 text-xs font-bold text-slate-900 outline-none focus:border-primary"
                            />
                          </div>
                          <button
                            type="button"
                            onClick={() => handleCounter(item.id)}
                            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl transition shadow-md shadow-blue-600/20 shrink-0"
                          >
                            Send Counter
                          </button>
                        </div>
